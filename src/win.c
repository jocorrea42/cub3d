		...
		// draw the wall
{
	...
	x_o = get_x_o(mlx, current_texture);
		// seria la linea de la textura correspondiente en relacion a la imagen o sea donde estaria
...
void	render_wall(t_cub *mlx, int ray)
{
	...
	mlx->ray->distance *= cos(nor_angle(mlx->ray->ray_angle
			- mlx->ply->angle));          // fix the fisheye
	wall_h = (*mlx->tile / mlx->ray->distance) * ((S_W / 2)
			/ tan(mlx->ply->fov_rd / 2)); // get the wall height
	b_pix = (S_H / 2) + (wall_h
			/ 2);                                                    
		// get bottom pixel
	t_pix = (S_H / 2) - (wall_h
